...
// Demo courses — fallback (admin e Full Courses video add korle DB theke ashe)
const demoCourses = [
  { id: 1, title: "Forex Basics", dur: "12:30", lessons: 5, img: "https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=640&h=360&fit=crop", descKey: "fcCourse1", video_url: "" },
  { id: 2, title: "Technical Analysis", dur: "18:45", lessons: 8, img: "https://images.unsplash.com/photo-1642790106117-e829e14a795f?w=640&h=360&fit=crop", descKey: "fcCourse2", video_url: "" },
  { id: 3, title: "Risk Management", dur: "09:20", lessons: 4, img: "https://images.unsplash.com/photo-1535320903710-d993d3d77d29?w=640&h=360&fit=crop", descKey: "fcCourse3", video_url: "" },
  { id: 4, title: "Advanced Strategies", dur: "22:10", lessons: 10, img: "https://images.unsplash.com/photo-1590283603385-17ffb3a7f29f?w=640&h=360&fit=crop", descKey: "fcCourse4", video_url: "" },
  { id: 5, title: "Price Action", dur: "15:40", lessons: 6, img: "https://images.unsplash.com/photo-1516245834210-c4c142787335?w=640&h=360&fit=crop", descKey: "fcCourse5", video_url: "" },
  { id: 6, title: "Trading Psychology", dur: "10:15", lessons: 4, img: "https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=640&h=360&fit=crop", descKey: "fcCourse6", video_url: "" },
]
...
export default function FullCoursesPage() {
  ...
  // DB theke Full Courses video (admin panel → Full Courses tab e add kora)
  ...
  useEffect(() => {
    ...
    // Full Courses category video load (admin panel theke)
    fetch("/api/videos?category=full", { cache: "no-store" })
      .then((r) => r.json())
      .then((j) => { if (j.ok) setDbCourses(j.videos || []) })
      .catch(() => {})
  ...
  // Admin e Full Courses video add korle DB theke ashe — nahole demo fallback
  const courses: any[] = dbCourses.length > 0 ? dbCourses : demoCourses
  const getDesc = (v: any) => (v.descKey ? t(v.descKey) : v.description || "")
  const getImg = (v: any) => v.img || "https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=640&h=360&fit=crop"
  ...
  return (
    <main className={`min-h-screen transition-colors duration-300 ${isDark ? "bg-dark-950" : "bg-gray-50"}`}>
      ...
      <section className="max-w-6xl mx-auto px-4 py-12">
        {playing !== null ? (
          (() => {
            const v = courses.find((c) => c.id === playing)!
            ...
            return (
              <div>
                ...
                {/* Real video player (admin video) — URL na thakle mock preview */}
                <div className="relative aspect-video rounded-2xl overflow-hidden bg-black mb-6 shadow-2xl">
                  {url && isYouTube ? (
                    <iframe src={url.replace("watch?v=", "embed/").replace("youtu.be/", "youtube.com/embed/")} title={v.title} className="absolute inset-0 w-full h-full" allowFullScreen allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" />
                  ) : url ? (
                    <video src={url} controls autoPlay className="absolute inset-0 w-full h-full" />
                  ) : (
                    <>
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={getImg(v)} alt={v.title} className="absolute inset-0 w-full h-full object-cover opacity-40" />
                ...
                <p className={`mt-1 ${isDark ? "text-gray-400" : "text-gray-600"}`}>{getDesc(v)}</p>
              ...
              {courses.map((v: any) => (
                ...
                >
                  <div className="relative aspect-video overflow-hidden">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={getImg(v)} alt={v.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                  ...
                  <div className="p-4">
                    ...
                    <div className={`text-xs mt-1 ${isDark ? "text-gray-400" : "text-gray-500"}`}>{getDesc(v)}</div>
                  </div>
                </button>
              ))}
            </div>
