          ...
          {signals.map((s, i) => (
            <div key={i} className="trading-card">
              ...
              <div className="border-t border-dark-700/50 pt-3 flex items-center justify-between">
                ...
                <span className={`signal-badge border ${s.status === "TP Hit" ? "bg-green-500/20 text-green-400 border-green-500/30" : "bg-blue-500/20 text-blue-400 border-blue-500/30"}`}>{s.status}</span>
              </div>
            </div>
          ))}
        </div>
