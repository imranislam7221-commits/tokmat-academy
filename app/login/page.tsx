...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data.ok) {
        showToast("Google login success! Redirecting...", "bg-green-600");
        setTimeout(() => { window.location.href = data.redirect || "/dashboard"; }, 800);
      } else {
      ...
      <div className="w-full max-w-md">
        ...
        <div className={`rounded-2xl p-8 shadow-elevated border transition-colors ${isDark ? "bg-dark-800 border-dark-700" : "bg-white border-gray-100"}`}>
            ...
            <div>
              ...
              <div className="relative">
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@gmail.com"
                  className={`w-full px-4 py-3 pl-10 rounded-xl border outline-none transition-all text-sm ${isDark ? "bg-dark-700 border-dark-600 text-white placeholder-gray-500" : "border-gray-200"}`} />
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••"
                  className={`w-full px-4 py-3 pl-10 pr-10 rounded-xl border outline-none transition-all text-sm ${isDark ? "bg-dark-700 border-dark-600 text-white placeholder-gray-500" : "border-gray-200"}`} />
                <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="flex items-center gap-3 my-6">
            ...
            <span className={`text-xs font-medium ${isDark ? "text-gray-500" : "text-gray-400"}`}>{translate(locale, "or")}</span>
            <div className={`flex-1 h-px ${isDark ? "bg-dark-600" : "bg-gray-200"}`} />
          </div>

          <button onClick={handleGoogleLogin} className={`w-full flex items-center justify-center gap-3 border font-medium py-3 rounded-xl transition-all text-sm ${isDark ? "bg-dark-700 border-dark-600 text-gray-300 hover:bg-dark-600" : "bg-white border-gray-200 text-gray-700 hover:bg-gray-50"}`}>
            ...
            {translate(locale, "continueWithGoogle")}
          </button>
